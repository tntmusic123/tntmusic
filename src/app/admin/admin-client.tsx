...
export default function AdminPage() {
  ...
  // Local string state for comma-separated categories/fields
  const [noteCategoriesStr, setNoteCategoriesStr] = useState("");
  const [artistFieldsStr, setArtistFieldsStr] = useState("");
  
  // Note Form State
  const [isComposingNote, setIsComposingNote] = useState(false);
  const [noteForm, setNoteForm] = useState({ title: "", category: "Notice", content: "", coverUrl: "", isMainExposed: false });
  ...
  const partnerLogosInputRef = useRef<HTMLInputElement>(null);
  ...
  const [isUploadingPartnerLogos, setIsUploadingPartnerLogos] = useState(false);
  const [isUploadingNoteBodyImage, setIsUploadingNoteBodyImage] = useState(false);
  const [isUploadingOgImage, setIsUploadingOgImage] = useState(false);
  
  // Category/Field Management State
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        setNoteCategoriesStr((fetchedSettings.noteCategories || []).join(", "));
        setArtistFieldsStr((fetchedSettings.artistFields || []).join(", "));
  ...
  const saveSettings = async () => {
    await updateSiteSettings({ 
      ...
      noteCategories: noteCategoriesStr.split(',').map(s => s.trim()).filter(Boolean),
      artistFields: artistFieldsStr.split(',').map(s => s.trim()).filter(Boolean),
      partnerLogos: settings.partnerLogos || [],
      snsInstagram: settings.snsInstagram || "",
      snsYoutube: settings.snsYoutube || "",
      snsBlog: settings.snsBlog || "",
      snsKakao: settings.snsKakao || "",
    });
  ...
  const handleSettingsImageUpload = async (field: "aboutImages" | "studioImages" | "partnerLogos", e: React.ChangeEvent<HTMLInputElement>) => {
    const files = e.target.files;
    if (!files || files.length === 0) return;

    if (field === "aboutImages") setIsUploadingAboutImages(true);
    else if (field === "studioImages") setIsUploadingStudioImages(true);
    else setIsUploadingPartnerLogos(true);
    ...
    else if (field === "studioImages") setIsUploadingStudioImages(false);
    else setIsUploadingPartnerLogos(false);
  };

  const removeSettingsImage = (field: "aboutImages" | "studioImages" | "partnerLogos", index: number) => {
    const current = [...(settings[field] || [])];
  ...
  const handleOgImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (url) {
      ...
      toast.success("공유 이미지가 업로드되었습니다. 저장 버튼을 눌러 확정해주세요.");
    } else {
      toast.error("이미지 업로드에 실패했습니다.");
    }
    ...
    try {
      if (editingNoteId) {
        await updateNote(editingNoteId, {
          ...
          isMainExposed: noteForm.isMainExposed
        });
      ...
      setNoteForm({ title: "", category: "Notice", content: "", coverUrl: "", isMainExposed: false });
    } catch (err: any) {
      console.error(err);
  ...
  const handleNoteEdit = (note: Note) => {
    ...
    setNoteForm({
      ...
      isMainExposed: note.isMainExposed
    });
      ...
      <main className="flex-1 flex flex-col overflow-hidden bg-slate-950 relative">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="space-y-8">
                ...
                <Card className="bg-slate-900/60 border-slate-800/80 shadow-xl backdrop-blur-xl">
                  ...
                  <CardContent className="space-y-5 pt-6">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                      ...
                      <div className="space-y-2">
                        <Label className="text-xs text-slate-400 font-bold uppercase tracking-wider flex items-center gap-2"><Mail className="w-3.5 h-3.5 text-primary"/> 대표 이메일</Label>
                        <Input
                ...
                <Card className="bg-slate-900/60 border-slate-800/80 shadow-xl backdrop-blur-xl">
                  <CardHeader className="border-b border-slate-800/50 pb-5">
                    <CardTitle className="flex items-center gap-2 text-lg text-white font-bold">
                      <span className="w-1.5 h-5 bg-primary rounded-full" /> SNS 및 외부 링크 설정
                    </CardTitle>
                    <p className="text-xs text-slate-400 mt-2">웹사이트 푸터 및 연락처 페이지에 노출될 소셜 미디어 링크입니다.</p>
                  </CardHeader>
                  <CardContent className="space-y-5 pt-6">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                      <div className="space-y-2">
                        <Label className="text-xs text-slate-400 font-bold tracking-wider">인스타그램 URL</Label>
                        <Input
                          placeholder="https://instagram.com/..."
                          value={settings.snsInstagram || ""}
                          onChange={(e) => setSettings({ ...settings, snsInstagram: e.target.value })}
                          className="bg-slate-950/50 border-slate-700/50 text-white h-11 focus-visible:ring-primary/50 placeholder:text-slate-600 rounded-xl"
                        />
                      </div>
                      <div className="space-y-2">
                        <Label className="text-xs text-slate-400 font-bold tracking-wider">유튜브 URL</Label>
                        <Input
                          placeholder="https://youtube.com/..."
                          value={settings.snsYoutube || ""}
                          onChange={(e) => setSettings({ ...settings, snsYoutube: e.target.value })}
                          className="bg-slate-950/50 border-slate-700/50 text-white h-11 focus-visible:ring-primary/50 placeholder:text-slate-600 rounded-xl"
                        />
                      </div>
                      <div className="space-y-2">
                        <Label className="text-xs text-slate-400 font-bold tracking-wider">네이버 블로그 URL</Label>
                        <Input
                          placeholder="https://blog.naver.com/..."
                          value={settings.snsBlog || ""}
                          onChange={(e) => setSettings({ ...settings, snsBlog: e.target.value })}
                          className="bg-slate-950/50 border-slate-700/50 text-white h-11 focus-visible:ring-primary/50 placeholder:text-slate-600 rounded-xl"
                        />
                      </div>
                      <div className="space-y-2">
                        <Label className="text-xs text-slate-400 font-bold tracking-wider">카카오톡 채널 URL</Label>
                        <Input
                          placeholder="https://pf.kakao.com/..."
                          value={settings.snsKakao || ""}
                          onChange={(e) => setSettings({ ...settings, snsKakao: e.target.value })}
                          className="bg-slate-950/50 border-slate-700/50 text-white h-11 focus-visible:ring-primary/50 placeholder:text-slate-600 rounded-xl"
                        />
                      </div>
                    </div>
                ...
                <Card className="bg-slate-900/60 border-slate-800/80 shadow-xl backdrop-blur-xl shrink-0">
                  ...
                  <CardContent className="space-y-5 pt-6">
                    <div className="space-y-2">
                      ...
                      <Input
                        placeholder="TNT Music | 클래식 전문 연습실"
                    ...
                    <div className="space-y-2">
                       <Label className="text-xs text-slate-400 font-bold uppercase tracking-wider">공유 이미지 (OG Image)</Label>
                       <div className="flex flex-col gap-4">
                         {settings.ogImageUrl && (
                           <div className="relative w-full aspect-[1.91/1] max-w-md rounded-xl overflow-hidden border border-slate-700/50 bg-slate-950">
                             <img src={settings.ogImageUrl} className="w-full h-full object-cover" alt="OG Preview" />
                             <button 
                         ...
                         <div className="flex gap-2">
                           ...
                           <Button 
                             ...
                             className="h-11 px-4 rounded-xl bg-slate-800 hover:bg-slate-700 flex-shrink-0"
                           >
                             <Upload className="h-4 w-4" />
                           </Button>
                         </div>
                       </div>
                       <p className="text-[10px] text-slate-500 italic">※ 카카오톡, 페이스북 공유 시 노출될 1200x630 권장 이미지입니다.</p>
                    </div>
                  </CardContent>
                </Card>
              </div>
              ...
              <div className="space-y-8 flex flex-col">
                ...
                <Card className="bg-slate-900/60 border-slate-800/80 shadow-xl backdrop-blur-xl">
                  <CardHeader className="border-b border-slate-800/50 pb-5">
                    <CardTitle className="flex items-center gap-2 text-lg text-white font-bold">
                      <span className="w-1.5 h-5 bg-primary rounded-full" /> 협업 파트너사 로고 관리
                    </CardTitle>
                    <p className="text-xs text-slate-400 mt-2">메인 페이지 하단 슬라이더에 노출될 파트너사 로고들입니다.</p>
                  </CardHeader>
                  <CardContent className="pt-6">
                      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                        {(settings.partnerLogos || []).map((img, idx) => (
                          <div key={idx} className="group relative aspect-video rounded-xl overflow-hidden border border-slate-700/50 bg-white shadow-inner flex items-center justify-center p-4">
                             <img src={img} className="max-w-full max-h-full object-contain group-hover:scale-110 transition-transform duration-500" alt="Partner Logo" />
                             <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity" />
                             <button 
                               onClick={() => removeSettingsImage("partnerLogos", idx)}
                               className="absolute top-2 right-2 bg-red-500/90 hover:bg-red-500 backdrop-blur-md text-white p-1.5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity transform scale-90 group-hover:scale-100"
                             >
                               <X className="h-3 w-3" />
                             </button>
                        ...
                        <button 
                         onClick={() => partnerLogosInputRef.current?.click()}
                         disabled={isUploadingPartnerLogos}
                        ...
                        >
                          <Plus className="h-8 w-8 mb-2 group-hover:scale-110 transition-transform" />
                          <span className="text-xs font-medium">{isUploadingPartnerLogos ? "처리 중..." : "로고 추가"}</span>
                        </button>
                        <input 
                         ref={partnerLogosInputRef}
                         type="file" multiple accept="image/*" className="hidden" 
                         onChange={(e) => handleSettingsImageUpload("partnerLogos", e)}
                        />
                      </div>
                  </CardContent>
                </Card>
                  ...
                  <div className="space-y-6">
                    <div className="space-y-2">
                      ...
                      <Textarea
                        ...
                        value={noteCategoriesStr}
                        onChange={(e) => setNoteCategoriesStr(e.target.value)}
                        rows={2}
                        className="bg-slate-950/50 border-slate-700/50 text-white focus-visible:ring-primary/50 placeholder:text-slate-600 rounded-xl custom-scrollbar"
                      />
                      <p className="text-[11px] text-slate-500">쉼표(,)로 구분하여 카테고리 항목을 입력합니다. (예: 공지사항, 리뷰)</p>
                    </div>
                    <div className="space-y-2">
                      <Label className="text-xs text-slate-300 font-bold uppercase tracking-wider">아티스트 활동 분야 모음</Label>
                      <Textarea
                        placeholder="성악, 뮤지컬, 악기, 기타"
                        value={artistFieldsStr}
                        onChange={(e) => setArtistFieldsStr(e.target.value)}
                        rows={2}
                        className="bg-slate-950/50 border-slate-700/50 text-white focus-visible:ring-primary/50 placeholder:text-slate-600 rounded-xl custom-scrollbar"
                      />
                      <p className="text-[11px] text-slate-500">쉼표(,)로 구분하여 아티스트 분야를 입력합니다. (예: 성악, 뮤지컬배우)</p>
                    </div>
                  </div>
                </div>
              </CardContent>
              ...
              <div className="flex gap-3">
                ...
                {!isComposingNote && (
                  <Button onClick={() => {
                    setIsComposingNote(true);
                    setEditingNoteId(null);
                    setNoteForm({ title: "", category: settings.noteCategories?.[0] || "Notice", content: "", coverUrl: "", isMainExposed: false });
            ...
            {isComposingNote ? (
              <Card className="bg-slate-900/80 border-slate-800/80 shadow-2xl backdrop-blur-xl">
                ...
                <CardContent className="space-y-6 pt-6">
                  ...
                  <div className="flex items-center space-x-3 p-4 bg-slate-950/30 rounded-xl border border-slate-800/50 w-max cursor-pointer hover:bg-slate-950/50 transition-colors" onClick={() => setNoteForm(prev => ({...prev, isMainExposed: !prev.isMainExposed}))}>
                    <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${noteForm.isMainExposed ? "bg-primary border-primary" : "border-slate-700 bg-slate-900"}`}>
                      {noteForm.isMainExposed && <Check className="w-3.5 h-3.5 text-black font-bold" />}
                    </div>
                    <Label className="text-sm text-slate-300 font-medium cursor-pointer">메인 페이지에 노출하기</Label>
                  </div>
                  ...
                  <div className="space-y-3 pt-2">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-[10px] text-primary/70 font-medium">유튜브 링크를 붙여넣으면 자동으로 영상이 삽입됩니다.</span>
                    </div>
                    <div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
            <Card className="w-full max-w-md bg-slate-900 border-slate-800 shadow-2xl">
              <CardHeader className="border-b border-slate-800/50 pb-4 flex flex-row items-center justify-between space-y-0">
                <CardTitle className="text-lg text-white font-bold">노트 카테고리 관리</CardTitle>
                <Button variant="ghost" size="icon" onClick={() => setIsEditingCategories(false)} className="text-slate-400 hover:text-white h-8 w-8">
                  <X className="h-4 w-4" />
                </Button>
              </CardHeader>
              <CardContent className="pt-6 space-y-6">
                <div className="flex gap-2">
                  <Input 
                    ...
                    className="bg-slate-950/50 border-slate-700/50 text-white h-10 focus-visible:ring-primary/50 text-sm rounded-xl"
                  />
                  <Button onClick={() => {
                    if (!newCategoryName) return;
                    const updated = [...(settings.noteCategories || []), newCategoryName];
                    setSettings({ ...settings, noteCategories: updated });
                    setNoteCategoriesStr(updated.join(", ")); // Synchronize string state
                    updateSiteSettings({ noteCategories: updated });
                    setNewCategoryName("");
                    toast.success("추가되었습니다.");
                  }} className="bg-primary hover:bg-primary/90 text-black font-bold rounded-xl h-10 px-4 shrink-0">추가</Button>
                </div>
                
                <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                  {(settings.noteCategories || []).map((cat: string, idx: number) => (
                    <div key={idx} className="flex items-center justify-between p-3 bg-slate-950/50 rounded-xl border border-slate-800/50">
                      ...
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-slate-500 hover:text-red-400" onClick={() => {
                        const updated = (settings.noteCategories || []).filter((_: any, i: number) => i !== idx);
                        setSettings({ ...settings, noteCategories: updated });
                        setNoteCategoriesStr(updated.join(", ")); // Synchronize string state
                        updateSiteSettings({ noteCategories: updated });
        ...
        {isEditingFields && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
            <Card className="w-full max-w-md bg-slate-900 border-slate-800 shadow-2xl">
              <CardHeader className="border-b border-slate-800/50 pb-4 flex flex-row items-center justify-between space-y-0">
                <CardTitle className="text-lg text-white font-bold">아티스트 분야 관리</CardTitle>
                <Button variant="ghost" size="icon" onClick={() => setIsEditingFields(false)} className="text-slate-400 hover:text-white h-8 w-8">
                  <X className="h-4 w-4" />
                </Button>
              </CardHeader>
              <CardContent className="pt-6 space-y-6">
                <div className="flex gap-2">
                  ...
                  <Button onClick={() => {
                    ...
                    const updated = [...(settings.artistFields || []), newFieldName];
                    setSettings({ ...settings, artistFields: updated });
                    setArtistFieldsStr(updated.join(", ")); // Synchronize string state
                ...
                <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                  {(settings.artistFields || []).map((f: string, idx: number) => (
                    <div key={idx} className="flex items-center justify-between p-3 bg-slate-950/50 rounded-xl border border-slate-800/50">
                      <span className="text-slate-200 text-sm">{f}</span>
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-slate-500 hover:text-red-400" onClick={() => {
                        const updated = (settings.artistFields || []).filter((_: any, i: number) => i !== idx);
                        setSettings({ ...settings, artistFields: updated });
                        setArtistFieldsStr(updated.join(", ")); // Synchronize string state
                        updateSiteSettings({ artistFields: updated });
                        toast.success("삭제되었습니다.");
