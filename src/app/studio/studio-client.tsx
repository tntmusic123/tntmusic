        ...
        <div className="mx-auto max-w-6xl px-6 lg:px-12">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            ...
            <div className="rounded-2xl overflow-hidden border border-border group">
              <div className="h-80 sm:h-[400px] bg-background relative overflow-hidden">
                {settings?.studioImages && settings.studioImages.length > 0 ? (
                  <PremiumCarousel images={settings.studioImages} />
                ) : (
                  <div className="w-full h-full flex items-center justify-center relative">
                    <div className="absolute inset-0 bg-gradient-dark opacity-50" />
                    <svg className="w-16 h-16 text-primary/30 relative z-10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round">
              ...
              <div className="p-8 bg-background">
                <h2 className="text-2xl font-bold text-foreground mb-1">티엔티뮤직스튜디오</h2>
                <p className="text-sm text-primary tracking-wider mb-5">Premium Practice Room</p>
                <div className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
                  <div className="flex items-center gap-2 text-muted-foreground">
                    <Maximize className="h-4 w-4 text-primary" /> 약 40평
                  </div>
                  <div className="flex items-center gap-2 text-muted-foreground">
                    <Piano className="h-4 w-4 text-primary" /> 그랜드 피아노
                  </div>
                  <div className="flex items-center gap-2 text-muted-foreground">
                    <Shield className="h-4 w-4 text-primary" /> 방음시설
                  </div>
                  <div className="flex items-center gap-2 text-muted-foreground">
                    <Volume2 className="h-4 w-4 text-primary" /> 음향시설
                  </div>
                  <div className="flex items-center gap-2 text-muted-foreground">
                    <Clock className="h-4 w-4 text-primary" /> 24시간 운영
                  </div>
                  <div className="flex items-center gap-2 text-muted-foreground">
                    <Thermometer className="h-4 w-4 text-primary" /> 에어컨
                  </div>
                  <div className="flex items-center gap-2 text-muted-foreground">
                    <Wind className="h-4 w-4 text-primary" /> 공기청정기
                  </div>
                  <div className="flex items-center gap-2 text-muted-foreground">
                    <Droplets className="h-4 w-4 text-primary" /> 정수기
                  </div>
                </div>

                <div className="flex flex-wrap gap-1.5 mt-6 mb-6">
                  {["그랜드 피아노", "방음시설", "음향시설", "24시간 운영", "에어컨", "공기청정기", "정수기", "보면대 및 의자(60개보유)", "24시간"].map((f) => (
                    <span key={f} className="text-[10px] px-2 py-0.5 rounded-full bg-muted text-muted-foreground border border-border">{f}</span>
                  ))}
                </div>
